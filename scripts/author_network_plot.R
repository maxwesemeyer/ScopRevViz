...
nodes <- data.frame(id = 1:length(paper.statnet%v%"vertex.names"),
                         ...
                         papers = paper_count$Papers) ## NAMEN ÜBERPRÜFEN HIER

# Interactive Network

# Nodes-Size-Skalierung
#nodes$size <- 20 + 40 * (scale(nodes$papers) - min(scale(nodes$papers))) / (max(scale(nodes$papers)) - min(scale(nodes$papers)))
#nodes$size_log <- 20 + 40 * (log(nodes$papers) - min(log(nodes$papers))) / (max(log(nodes$papers)) - min(log(nodes$papers)))
...
plot_interactive <- visNetwork(nodes = nodes, edges = edges, main = "Co-Author Network", width = 900, height = 900) %>% 
  ...
  visOptions(nodesIdSelection = list(enabled  = TRUE, useLabels = TRUE, main = "Select by Author"),
             selectedBy = "papers") # 2 Dropdown Liste
...
# igraph instead of statnet package --> NO LO PUEDO PERSONALIZAR
